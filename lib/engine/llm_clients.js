...
export class LLMGateway {
  ...
  // Track 014: resolves the effective Gemini "thinking level" for a call.
  // Precedence: per-call override (including an explicit null, meaning "no
  // thinkingConfig at all") > engine-level config.llmThinkingLevel (same
  // explicit-null semantics) > package default 'MINIMAL'. Real measurement
  // (coachai, a representative real prompt, no max_tokens cap): default
  // (medium, ~2,800-3,600 thinking tokens/call) averaged ~27s across 3
  // trials; MINIMAL averaged ~12s — 45-70% faster on every trial, with no
  // observed output-quality or marker-compliance loss. thinkingBudget (the
  // token-count control) was tried first and found unreliable for this
  // model — thinkingLevel is the model-respected lever. Using `in` rather
  // than `??` because `undefined` (not set) and `null` (explicitly opted
  // out) must resolve differently.
  resolveThinkingLevel(optionsLevel) {
    if (optionsLevel !== undefined) return optionsLevel;
    if ('llmThinkingLevel' in this.config) return this.config.llmThinkingLevel;
    return 'MINIMAL';
  }
  ...
  async *streamExecuteGemini(modelName, messages, options) {
    ...
    const stream = await aiClient.models.generateContentStream({
      ...
      config: {
        ...
        // Track 014: see resolveThinkingLevel()'s comment for the rationale
        // and measured impact.
  ...
  async *streamExecuteVertex(modelName, messages, options) {
    ...
    const stream = await ai.models.generateContentStream({
      ...
      config: {
        ...
        abortSignal: options.abortSignal,
        // Track 014: see resolveThinkingLevel()'s comment for rationale/impact.
        ...(thinkingLevel != null ? { thinkingConfig: { thinkingLevel } } : {}),
      }
    });
