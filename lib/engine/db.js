...
export async function initDb(config) {
  ...
  if (dbType === 'sqlite') {
    ...
    db = await open({
      ...
      driver: sqlite3.default ? sqlite3.default.Database : sqlite3.Database,
    });

    await db.exec(getSchema('sqlite'));
    await runMigrations('sqlite', db);
    logger.info({ sqlitePath: config.sqlitePath }, '[TokenTalos] SQLite initialized');
  } else {
    const { Pool } = pg;
    db = new Pool({
      host: config.pgHost,
    ...
    db.on('connect', (client) => {
      client.query(`SET search_path TO ${schemaName}`).catch(err => {
        logger.error({ err }, '[TokenTalos] Error setting search_path on connect');
      });
    });

    logger.info({ pgHost: config.pgHost, pgPort: config.pgPort, schemaName }, '[TokenTalos] PostgreSQL initialized');
  }
...
async function runMigrations(type, database, schemaName = '') {
  ...
  if (type === 'sqlite') {
    ...
    try {
      ...
      logger.warn({ err: e }, '[TokenTalos] Migration check failed (Postgres)');
    }
  }
}
