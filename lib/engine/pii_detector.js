...
const PII_PATTERNS = [
  {
    // Local-part and domain quantifiers are bounded (RFC 5321: local
    // part <= 64 octets, domain <= 253) to prevent catastrophic
    // backtracking on long non-matching runs -- an unbounded `+` here
    // is O(n^2) on any long alphanumeric-ish string with no `@`.
    type: 'email',
    regex: /[a-zA-Z0-9._%+-]{1,64}@[a-zA-Z0-9.-]{1,253}\.[a-zA-Z]{2,}/g
  },
  {
    type: 'api_key',
