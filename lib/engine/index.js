...
export class TokenTalosEngine {
  constructor(config = {}) {
    ...
    this.logger = this.config.logger || getLogger();
    this.db = null;
    this.cache = null;
    this.opv = null;
  ...
  async execute(params) {
    ...
    // 1. Process Parts
    const { processedParts, metadata, criticalThreatFound } = await this.process(parts);

    if (criticalThreatFound && this.config.securityAction === 'reject') {
    ...
    const result = await gateway.execute(finalProvider, finalModel, messages, {
      ...
      // Track 013: lets a caller cancel an in-flight call (e.g. on its own
      // client disconnect) instead of it running to natural completion.
      abortSignal: params.abortSignal || options.abortSignal,
    });
  ...
  async *streamExecute(params) {
    if (!this.initialized) await this.init();
    const { provider, model, parts, endpoint, options = {}, orgId, projectId } = params;
    const startTime = Date.now();

    const finalOrgId = orgId || this.orgId;
    const finalProjectId = projectId || this.projectId;

    // Process parts (PII, etc) before streaming
    const { processedParts } = await this.process(parts);
    const finalProvider = provider || this.config.llmProvider || 'gemini';
    const finalModel = model || this.config.defaultModel || 'gemini-3-flash-preview';
    ...
    try {
      for await (const chunk of gateway.streamExecute(finalProvider, finalModel, messages, {
        ...options,
        gcpProjectId: params.gcpProjectId || options.gcpProjectId,
        // Track 013: see execute()'s identical field for rationale.
        abortSignal: params.abortSignal || options.abortSignal,
      })) {
        // Track 011: gateway.streamExecute() yields {text, usageMetadata} objects
        // (not bare strings) so usage can be tracked without buffering chunks —
        // only the last chunk of a stream typically carries real token counts.
        const text = typeof chunk === 'string' ? chunk : chunk.text;
        const usageMetadata = typeof chunk === 'object' ? chunk?.usageMetadata : null;
      ...
      // Persist once the stream completes (or errors, best-effort — see plan.md
      // Phase 3). Deliberately skips cache read/write, variable_actions,
      // security_alerts, and explain_plans — not at parity with execute() yet.
      try {
        const latencyMs = Date.now() - startTime;
        const inputTokens = lastUsage?.promptTokenCount || lastUsage?.prompt_tokens || 0;
        const outputTokens = lastUsage?.candidatesTokenCount || lastUsage?.completion_tokens || 0;
        const calculator = getCostCalculator();
        const [inputCost, outputCost] = calculator.calculateCost(
          finalProvider, finalModel, inputTokens, outputTokens
        );
        const db = this.getDb();
        const limitExceeded = (inputTokens + outputTokens) > (this.config.maxTokens || 32000);

        await db.run(`
          INSERT INTO usage_data (id, org_id, project_id, type, provider, model, full_prompt, response_content, input_tokens, output_tokens, total_tokens, input_cost, output_cost, total_cost, endpoint, latency_ms, token_limit_exceeded, timestamp)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [
          trackingData.id, finalOrgId, finalProjectId, 'execution', finalProvider, finalModel, fullPromptString, fullResponse, inputTokens, outputTokens,
          inputTokens + outputTokens, inputCost, outputCost, inputCost + outputCost,
          endpoint, latencyMs, limitExceeded ? 1 : 0, trackingData.timestamp
        ]);
        ...
        // Track 011 follow-up: a real ~76s streamed call completed
        // successfully but left no usage_data row, and a plain console.warn
        // turned out to be unrecoverable — consumers that pipe stdout
        // straight into a structured-log viewer (e.g. `node ... | pinorama`)
        // silently drop plain-text lines, so a persist failure like this one
        // left no trace anywhere. Track 012: log via pino (now actually
        // surfaced by pino-aware viewers) AND keep the durable fallback file
        // as defense in depth — this is exactly the kind of failure worth
        // over-logging.
        const failureDetail = {
          usageId: trackingData?.id,
          endpoint,
          provider: finalProvider,
          model: finalModel,
        ...
        this.logger.error(failureDetail, 'Failed to persist streamExecute usage');
        try {
          const logPath = this.config.errorLogPath || path.join(os.tmpdir(), 'tokentalos-stream-errors.log');
          const entry = {
            ...
            model: finalModel,
            durationMs: Date.now() - startTime,
            error: { name: persistErr.name, message: persistErr.message, stack: persistErr.stack },
          };
          fs.appendFileSync(logPath, JSON.stringify(entry) + '\n');
        } catch (_) { /* logging is best-effort only — never let it throw */ }
      }
    }
  }
